...
public class FileSynchronizerTodoTaskTreeNode extends FileSynchronizationStatusTreeNode {
    ...
    /**
     * [<] Used when the source file has been modified outside. (is older than
     * the backup)
     */
    public static final int TODO_UPDATE_SOURCE = 1;

    /**
     * [-] Used when a source file has to be deleted (it no more exists in
     * destination).
     */
    public static final int TODO_DELETE_SOURCE = 2;

    /**
     * [+] Used when the source file has to be created (it does not exist in
     * destination - must never happen).
     */
    ...
    /**
     * [-] Used when a destination file has to be deleted (it no more exists in
     * source).
     */
